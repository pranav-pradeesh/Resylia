...
type OrgSlack = {
  ...
  manager_slack_channel_id: string | null
}
...
async function sendSlackNudge(orgId: string, nudges: any[]): Promise<void> {
  try {
    const { data: org, error } = await getAdminDb()
      ...
      .eq('id', orgId)
      .single() as unknown as { data: OrgSlack | null; error: any }
    ...
    if (highPriorityNudges.length > 0) {
      ...
      for (const [issue, nudges] of Object.entries(groupedByIssueType)) {
        const message = createSlackMessage(nudges)
...
function createSlackMessage(nudges: any): string {
  ...
  const issueTypes = nudges.reduce((acc: any, n: any) => {
  ...
  const issueType = Object.keys(issueTypes)[0]
  const groupedNudges = issueTypes[issueType]
  const names = groupedNudges.map((n: any) => n.employee_name).join(', ')
  const descriptions = groupedNudges.map((n: any) => n.issue_description).join('; ')

  return `🚨 Manager Alert: ${names}\n\nIssues:\n• ${descriptions}\n\nRecommended Actions:\n${groupedNudges.map((n: any) => n.action).join('\n- ')}\n\nPlease check in with these team members today and take preventive action.`
}
...
export async function sendSlackMessage({ channel, blocks }: { channel: string; blocks: any[] }) {
  try {
    const { data: org } = await getAdminDb()
      ...
      .limit(1)
      .single() as unknown as { data: { slack_bot_token: string | null } | null; error: any }

    if (!org || !org.slack_bot_token) {
  ...
  try {
    ...
    const response = await fetch('https://slack.com/api/chat.postMessage', {
      ...
      body: JSON.stringify({
        ...
        blocks: [
          { type: 'header', text: { type: 'plain_text', text: `🚨 ${alert.severity.toUpperCase()} Escalation Alert` } },
          { type: 'section', fields: [{ type: 'mrkdwn', text: `*Department:*\n${alert.department}` }, { type: 'mrkdwn', text: `*Severity:*\n${alert.severity}` }] },
          { type: 'section', text: { type: 'mrkdwn', text: `*Issue:*\n${alert.issue}` } },
          { type: 'context', elements: [{ type: 'mrkdwn', text: `Timestamp: ${alert.timestamp}` }] },
        ],
      }),
    })
